...
_SIM_META = [
    ...
    {
        "id": "11_longshort_horizons",
        "title": "공매도 × 지평별 비교 (30일 ~ 2년)",
        "subtitle": "단기·중기에선 공매도가 도움 될까?",
        "description": (
            "시뮬 10은 8.5년 장기였으나, 단기·중기(30/60/90/365/730일)에서 공매도가 어떻게 작동하는지 별도 검증. "
            "각 지평마다 long / short / long+short 3모드 × 10코인 = 15개 결과."
        ),
        "chart": "11_longshort_horizons.png",
        "primary_csv": "11_longshort_horizons.csv",
        "note": (
            "30~90일: 일봉 신호 희박으로 매매 거의 없음 (±0%). "
            "1년: short-only +0.67%로 유일 양수 (최근 알트 약세장). L+S는 -2.99%로 long-only(-3.66%)보다 소폭 개선. "
            "2년: long +6.70%, long+short +6.81% — 수익은 동등하지만 L+S 낙폭 -22.69%로 5%p 더 큼. "
            "결론: 지평에 따라 효과 달라짐 — 하락장 편중 시기(1년)엔 short이 유리, 장기(8.5년)엔 long이 압도. "
            "평균적으로는 long-only가 안정적이지만 '최근 1년'처럼 하락 구간에선 short 섞는 게 이익 방어."
        ),
    },
    {
        "id": "10_longshort",
