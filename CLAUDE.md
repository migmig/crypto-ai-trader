...
## 실행 흐름 (하이브리드: 알고리즘 + AI, v5 일봉 기반)

> **v5 변경**: 시뮬 06/07 검증으로 15분봉 의사결정이 노이즈·수수료 누적으로 장기 성과 열악임이 드러남. 일봉 기반으로 전환하고 사이클은 1시간 주기. 알고리즘이 먼저 규칙 기반 신호를 내고, **non-hold 신호가 있을 때만** Claude를 호출한다.

1. `run_cycle.sh` 실행 (macOS **launchd**로 1시간(3600초)마다 자동 실행)
...
- **포지션별 트레일링 손절**: 수익 구간에서 고점 대비 **-10%** 하락 시 매도 (v4 -7% → 완화)
- **전체 포트폴리오 백스톱**: 총 투자금 대비 **-25%** 도달 시 전량 매도 (v4 -15% → 완화, 그리드 서치 최적)
- 단일 코인 최대 비중: 전체 평가금의 50%
- 하루 최대 거래 횟수: **5회** (일봉 기준 → 빈도 낮음)
- 매수 후 최소 홀딩: **1440분(24시간)** (일봉 판단이라 장기 관점)
- **sell_strong 익절 조건**: RSI1d ≥ 70 + MACD 데드크로스 매도는 **평단 대비 +3% 이상 수익 구간**에서만 발동

### 분석 항목 (v5 — 일봉 중심)
1. **기술적 분석 (일봉)**: RSI1d(14), MACD1d(12,26,9), 볼린저1d(20,2), MA5/MA20(1d)
2. **거래량 분석**: 일봉 거래량 ÷ 20일 평균
3. **추세 분석**: 일봉 MA5 vs MA20 + 현재가 기준 상승/하락/횡보
4. **보조 (15m/1h)**: UI·AI 컨텍스트용. 판단 자체는 일봉.
5. **호가 분석**: 매수/매도 호가 두께, 스프레드 (AI 판단 시 고려)

### 매매 판단 기준 (v5 — 일봉 기반)
- **적극 매수**: RSI1d ≤ 35 + MACD1d 반등(골든크로스 OR 히스토그램 상승전환) + 거래량비율(1d) ≥ 1.3 → **평가금액 30%** 매수
- **매수 고려**: RSI1d ≤ 40 + 볼린저(1d) 하단 근접 + 일봉 추세 ≠ 하락 → **평가금액 10%** 소량 매수
- 여러 코인 동시 매수 신호 → 남은 현금 비율 분배
- **적극 매도**: RSI1d ≥ 70 + MACD1d 데드크로스 + **평단 대비 +3% 이상** → 전량 매도 (손실 구간에선 발동 안 함)
- **매도 고려**: RSI1d ≥ 65 + 볼린저(1d) 상단 근접 + 일봉 하락 추세 + **평단 대비 +3% 이상** → 50% 부분 매도
- **관망**: 위 조건 미충족 시 관망
...
| `com.migmig.crypto-trader-cycle` | 1시간마다 `run_cycle.sh` 실행 (신호 → 조건부 AI → 매매) | `StartInterval` 3600초 |
