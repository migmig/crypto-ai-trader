...
function Card({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div className="bg-[color:var(--color-apple-tile-2)] border border-white/[0.06] rounded-[18px] p-3 sm:p-6">
      <div className="text-[10px] sm:text-[11px] text-white/50 uppercase tracking-[0.08em] mb-1 sm:mb-2">{label}</div>
      {children}
    </div>
  )
}
    ...
    <div className="grid grid-cols-2 lg:grid-cols-5 gap-2 sm:gap-4">
      <Card label="Total Value">
        ...
        {data.liquidation_value !== undefined && (
          <div className="text-[10px] sm:text-xs text-right text-gray-500 mt-1 pt-1 border-t border-white/[0.06] tabular-nums">
